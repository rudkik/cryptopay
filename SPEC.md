...
- `POST /api/v1/invoices` — body: `amount` (required, >0), `currency` (USDT|USDC, optional), `network` (ethereum|bsc|tron, optional; либо обе, либо ни одной — одна без второй = 422), `external_id?`, `description?`, `customer_email?`, `customer_id?`, `metadata?` (object), `success_url?`, `cancel_url?`, `expires_in?` (сек, 60–86400; default — `settings.invoice_ttl` мерчанта, он же «Payment window» в админке, по умолчанию 3600). → 201 Invoice.
