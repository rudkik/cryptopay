...
### 5в. Список из первых N HD-адресов (рекомендуется вместе с 5б)

С одним лишь xpub каждый счёт получает новый адрес, и собирать деньги придётся с сотен адресов. Чтобы
обычные платежи крутились по небольшому известному набору, а свежие HD-адреса выдавались только на пике:

```bash
make seed-addresses          # первые 10 адресов каждой сети (индексы 0–9) → раздел Addresses
make seed-addresses N=20     # другое количество
make seed-addresses DRY=1    # только показать, ничего не записывать
```

Команда берёт xpub из админки (или из env watcher'а), добавляет адреса с меткой `HD #i` и приоритетом
по индексу, уже добавленные пропускает, а `next_index` сети поднимает до N, чтобы резервная выдача не
повторила адрес из списка. Повторный запуск безопасен. В кошельке-приложении (OKX Wallet и т.п.) это
аккаунты 1…N из той же сид-фразы.
